...
│
├── AGENTS.md                  # AI Agent 系统指令（客服角色 + 工作流程）
├── SOUL.md                    # Agent 人格/价值观定义
├── USER.md                    # 用户画像配置
├── HEARTBEAT.md               # 定时任务注册（nanobot cron）
│
...
│
├── skills/                    # Nanobot 技能定义
│   ├── faq/SKILL.md           # FAQ 查询技能（强制 exec 读取知识库）
│   └── ticket/SKILL.md        # 工单管理技能（5 步工作流）
│
...
Agent 内部按 `AGENTS.md` 的 5 步工作流执行：
1. 读取用户记忆（`memory/MEMORY.md`）
2. 执行意图路由指令
3. 查找知识库答案
4. 禁止编造（未读知识库就回答视为违规）
5. 更新用户记忆
